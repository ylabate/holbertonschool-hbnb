...
export default function Header({ isLoggedIn }) {
  return (
    <>
      <header className="app-header h-20 flex z-30 items-center p-3 rounded-b-3xl gap-4">
        <section className="flex-1 flex justify-start">
          <Link className="brand-mark" to="/">
            <img className="logo brand-logo" src="/logo.png" alt="hbnb logo" />
          </Link>
        </section>
        ...
        <section className="flex-1 flex justify-end">
          {isLoggedIn ? (
            ...
            <Link className="user-button login-button" to="/login">
              <Unplug size={30} />
            </Link>
          )}
        </section>
