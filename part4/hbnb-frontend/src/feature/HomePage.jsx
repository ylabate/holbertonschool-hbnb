  ...
  return (
    ...
    >
      {!isDeckReady || renderedPlaces.length === 0 ? (
        ...
        <section style={{ height: virtualHeight }}>
      ...
      {currentPlace ? (
        <>
          {fadingLayerStyle ? (
            <section
              className={`action-column-layer action-column-layer-fade fixed bottom-5 z-40 h-130 w-22 right-5 md:right-[max(1.25rem,calc((100vw-40rem)/2-2.5rem))] ${isLayerFadingOut ? "action-column-layer-hidden" : ""}`}
              style={fadingLayerStyle}
              aria-hidden="true"
            />
          ) : null}
          <section
            className="action-column-layer fixed bottom-5 z-[41] h-130 w-22 right-5 md:right-[max(1.25rem,calc((100vw-40rem)/2-2.5rem))]"
            style={activeLayerStyle || colorFromId(currentPlace.id)}
            aria-hidden="true"
          />
          <aside className="action-column fixed bottom-5 z-50 h-130 w-22 flex flex-col justify-between items-center py-3 right-5 md:right-[max(1.25rem,calc((100vw-40rem)/2-2.5rem))]">
            <Heart
